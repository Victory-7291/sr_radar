...
public:
    RosbagPlayer(const rclcpp::NodeOptions & options)
        ...
        image_publisher_ = this->create_publisher<sensor_msgs::msg::Image>("rosbag_image", rclcpp::SensorDataQoS());
...
private:
    void play_bag() {
        while (rclcpp::ok()) {
            ...
            // 处理 Image 消息
            //else if (bag_message->topic_name == "/rosbag_image") {
                ...
                // 输出图像尺寸和步长信息
                //RCLCPP_INFO(this->get_logger(), "Image width: %d, height: %d, step: %d", image_msg->width, image_msg->height, image_msg->step);
