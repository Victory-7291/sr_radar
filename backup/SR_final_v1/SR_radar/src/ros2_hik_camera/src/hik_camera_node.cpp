...
// OpenCV
#include <opencv2/opencv.hpp>  // 添加这行
...
public:
  explicit HikCameraNode(const rclcpp::NodeOptions & options) : Node("hik_camera", options)
  {
    ...
    camera_pub_ = image_transport::create_camera_publisher(this, "camera_image", rmw_qos_profile_sensor_data);
	...
	// Check buffer size before conversion
	  if (image_msg_.data.size() < img_info_.nHeightMax * img_info_.nWidthMax * 3) {
	      RCLCPP_ERROR(this->get_logger(), "Image buffer size is too small!");
	      return;
	  }
          ...
          //RCLCPP_INFO(this->get_logger(), "Image data size after conversion: %zu", image_msg_.data.size());
          ...
          //RCLCPP_INFO(this->get_logger(), "Image width: %d, height: %d, step: %d", image_msg_.width, image_msg_.height, image_msg_.step);
          //RCLCPP_INFO(this->get_logger(), "Image width: %d, height: %d, step: %d, data size: %zu", image_msg_.width, image_msg_.height, image_msg_.step, image_msg_.data.size());
          //std::cout<<"time: "<<image_msg_.header.stamp.sec<<"."<<image_msg_.header.stamp.nanosec<<std::endl;
...
private:
  void declareParameters()
  {
        ...
        double exposuretime;
    if (this->get_parameter("exposure_time", exposuretime))
    {
    RCLCPP_INFO(this->get_logger(), "Retrieved exposuretime: %f", exposuretime);
    }
    else
    {
    RCLCPP_WARN(this->get_logger(), "Failed to get exposure_time parameter. Using default value.");
	}
    ...
    //double gain = this->declare_parameter("gain", 14, param_desc);
