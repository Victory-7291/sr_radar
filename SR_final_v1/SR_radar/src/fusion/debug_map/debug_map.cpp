...
namespace tdt_radar {
    ...
    public:
        explicit DebugMap(const rclcpp::NodeOptions & options) : Node("debug_map", options){
            match_result_sub = this->create_subscription<vision_interface::msg::DetectResult>("/resolve_result", rclcpp::SensorDataQoS(), std::bind(&DebugMap::match_result_callback, this, std::placeholders::_1));
        ...
        void match_result_callback(const std::shared_ptr<vision_interface::msg::DetectResult> msg) {
            ...
            // 创建一个临时拷贝进行坐标系转换
            vision_interface::msg::DetectResult transformed_msg = *msg;
            
            // 坐标系转换和特殊处理（从kalman_filter.cpp中移植）
            if(self_color == radar_interface::team_color::C_BLUE){
                for(int i=0; i<6; i++){
                    if(transformed_msg.blue_x[i]!=0 && transformed_msg.blue_y[i]!=0){
                        transformed_msg.blue_x[i]=28-transformed_msg.blue_x[i];
                        transformed_msg.blue_y[i]=15-transformed_msg.blue_y[i];
                    }
                    if(transformed_msg.red_x[i]!=0 && transformed_msg.red_y[i]!=0){
                        transformed_msg.red_x[i]=28-transformed_msg.red_x[i];
                        transformed_msg.red_y[i]=15-transformed_msg.red_y[i];
                    }
                }
            }
            ...
            for (int i = 0; i < 6; i++) {
                if (transformed_msg.blue_x[i] != 0 && transformed_msg.blue_y[i] != 0) {
                    blue_point[i] = cv::Point2f(transformed_msg.blue_x[i], transformed_msg.blue_y[i]);
                    blue_update[i] = time; // 仅保留用于地图显示的时间戳
                }
                if (transformed_msg.red_x[i] != 0 && transformed_msg.red_y[i] != 0) {
                    red_point[i] = cv::Point2f(transformed_msg.red_x[i], transformed_msg.red_y[i]);
                    red_update[i] = time; // 仅保留用于地图显示的时间戳
                }
            }
            
            // 直接创建并填充MatchResult消息
            ...
            for (int i = 0; i < 6; i++) {
                ...
                if (transformed_msg.blue_x[i] != 0 && transformed_msg.blue_y[i] != 0) {
                    ...
                    match_result.blue[i].position[0] = transformed_msg.blue_x[i];
                    match_result.blue[i].position[1] = transformed_msg.blue_y[i];
                }
                ...
                if (transformed_msg.red_x[i] != 0 && transformed_msg.red_y[i] != 0) {
                    ...
                    match_result.red[i].position[0] = transformed_msg.red_x[i];
                    match_result.red[i].position[1] = transformed_msg.red_y[i];
                }
            }
