...
try:
    ...
    INTELLIGENT_PREDICTION_AVAILABLE = True
    print("✅ 智能预测算法模块已加载")
except ImportError:
    INTELLIGENT_PREDICTION_AVAILABLE = False
    print("⚠️  智能预测算法模块不可用，将使用简单预测")
    class Predict:  # 创建一个简单的占位符类
        def __init__(self):
            pass
...
# 初始化智能预测器 - 与ultra_radar保持一致
if INTELLIGENT_PREDICTION_AVAILABLE and config['blind_zone']['intelligent_prediction']['enabled']:
    print("🤖 启用智能预测算法")
    guess_predict = {
        "B1": Predict(),
        "B2": Predict(),
    ...
    INTELLIGENT_PREDICTION_ENABLED = True
else:
    print("📍 使用简单预测算法")
    guess_predict = {}
    INTELLIGENT_PREDICTION_ENABLED = False
...
class KalmanFilterWrapper:
    ...
    def add_data(self, name, x, y, threshold=100000.0):
        ...
        # 按照ultra_radar的方式更新智能预测器轨迹 - 时间间隔>1秒
        if INTELLIGENT_PREDICTION_ENABLED and name in guess_predict:
            try:
        ...
        self.last_update[name] = now_time
        ...
        for name in list(self.filters.keys()):
            if current_time - self.last_update[name] > self.max_inactive_time:
                ...
                if estimate is not None:
                    ...
                    # 按照ultra_radar的方式设置flag - 识别到机器人时设置flag=True
                    if INTELLIGENT_PREDICTION_ENABLED and name in guess_predict:
...
class SlidingWindowFilter:
    ...
    def add_data(self, name, x, y):
        ...
        if INTELLIGENT_PREDICTION_ENABLED and name in guess_predict:
            try:
                if self.last_update.get(name) and now_time - self.last_update[name] > 1:
                    guess_predict[name].add_point((x, y))
                    guess_predict[name].clear_point()
            except Exception as e:
                print(f"预测器更新失败: {e}")
...
def ser_send():
    ...
    def send_point_guess(send_name, guess_time_limit):
        ...
        if guess_value_now.get(send_name) < 120 and guess_value_now.get(send_name) - guess_value.get(
            ...
            if INTELLIGENT_PREDICTION_ENABLED and send_name in guess_predict:
                # 🤖 使用智能预测算法 - 与ultra_radar完全一致
                ...
                # 📍 使用简单预测算法 (原有逻辑)
                guess_index[send_name] = 1 - guess_index[send_name]
